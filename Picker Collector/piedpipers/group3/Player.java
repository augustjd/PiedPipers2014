...
public class Player extends piedpipers.sim.Player {
    ...
    public static class TargetStrategy extends Strategy {
        ...
        // sunyun add to decide whether the rats attracted by the piper are within a certain distance
        public boolean isWithinRadius(Player p, Scene s)
        {
        	double dist = 0;
        	Vector player_position = s.getPlayerPosition(p);
    		for(int i = 0; i < s.rats.size(); i++){
    			Vector v = s.rat_vectors.get(i);
    			dist = player_position.distanceTo(v);
    			//!!!!!!!!!!!!!!!!the number????
    			// 7 left 9, 7.5 left 6 
    			if(dist < radius_of_influence && dist > radius_of_stop_music) 
    			{
    				return false;
    			}
        ...
        public Point generateMove(Player p, Scene s) {
         	// sunyun add to stop and play music
  /*      	if(isWithinRadius(p, s))
        		p.music = false;
        	else
        		p.music = true;
        	*/
    ...
    // sunyun add ReturnToCollectorStragey
    public static class ReturnToCollectorStrategy extends TargetStrategy {
        ...
        public Point generateMove(Player p, Scene s) 
        {
        	...
        	this.target = s.pipers.get(p.id + 1);
            if (s.pipers.get(p.id).distanceTo(this.target) < 5)
            {
            	System.out.println("In the ReturnToCollector, arrive at collecotr");
                p.turnOffMusic();
                p.setStrategy(new PickerStrategy(p, s));
            } 
            else
            {
            	this.target = s.pipers.get(p.id + 1);
    ...
    // sunyun add for PickerStragegy
    public static class PickerStrategy extends TargetStrategy{
    	int rat_index;
    	double radius_of_handoff = 5.0;
    	int ticks_to_collector = 30;
    	
		public PickerStrategy(Player p, Scene s) {
			super(s.getGatePosition());
			this.rat_index = this.chooseRat(p, s);
			 ...
			 for(int i = 0; i < s.rats.size(); i++)
		        {
		        	...
		        	{
		        		for(int j = 0; j < s.pipers.size(); j++)
		        		{
		        			if(j != p.id)
		        			{
		        				double dist = s.rat_vectors.get(i).distanceTo(s.pipers.get(j));
		        				if(dist < radius_of_influence) // the rat is attracted by a piper
		        				{
		        		...
		        		{
		        			double dist = s.rat_vectors.get(i).distanceTo(s.pipers.get(p.id));
	    ...
	    public int chooseRat(Player p, Scene s) 
	    {
	        ...
	        public Point generateMove(Player p, Scene s) 
	        {
	            ...
	            if(p.id + 1 < s.pipers.size()) // the picker has a collector to help it
	            {
	            	double dist = s.pipers.get(p.id).distanceTo(s.pipers.get(p.id + 1));
	            	if(dist < radius_of_handoff)
	            	{
	            		System.out.println("---------handoff to the collector--------");
	            		p.turnOffMusic();
	            		this.rat_index = chooseRat(p, s);
	            	}
	            	else // decide whether should go to the collector
	 	            {
	 	        		...
	 	        		if(this.rat_index >= 0)
	 	        		{
		            		System.out.println("----------i am picker, what should i do, i am not new collector");
	 	        			...
	 	        			dis_picker_rat = s.pipers.get(p.id).distanceTo(s.rat_vectors.get(this.rat_index));
	 	        			if(p.id + 1 < s.pipers.size())
	 	        			{
	 	        			
	 	        				System.out.println("hi, i am the picker and I have a collector");
	 	        				dis_picker_collector = s.pipers.get(p.id).distanceTo(s.pipers.get(p.id + 1));
	 	        				dis_rat_collector    = s.rat_vectors.get(this.rat_index).distanceTo(s.pipers.get(p.id + 1));
    ...
    // sunyun add for CollectorStrategy
    public static class CollectorStrategy extends TargetStrategy{
    	int id;
...
/*    		if(s.getSide(me) == Scene.Side.RIGHT)
    ...
    public static class RadialGreedyStrategy extends TargetStrategy {
        ...
        public Point generateMove(Player p, Scene s) {
            ...
            // sunyun add 
            this.moving_towards_gate = false;
            // sunyun blog it
/*            if (rats_in_slice.size() == rats_influenced) {
    ...
    public static class Scene {
        ...
        // sunyun add for stop and play music
    	public static double radius_of_stop_music = 4.0;
	...
	// sunyun add for stop and play music
	static double radius_of_stop_music = 5.0;
	...
	public Point move(Point[] pipers, Point[] rats, boolean[] music) {
        ...
        if (strategy == null) {
            ...
            // sunyun change to add picker and collector strategy
            if(id % 2 == 0)
            {
            	this.setStrategy(new PhaseStrategy(Arrays.asList(
                        //new PhaseStrategy.Phase(0.00, new RatTargetStrategy(this, s)),
                        new PhaseStrategy.Phase(0.0007, new RadialGreedyStrategy(this, s)),
                        new PhaseStrategy.Phase(0.0000, new PickerStrategy(this, s)))));
                        //new PhaseStrategy.Phase(0.000, new VerticalSweepStrategy(this, s)))));
            }
            else
            {
            	this.setStrategy(new PhaseStrategy(Arrays.asList(
                        ...
                        new PhaseStrategy.Phase(0.0007, new RadialGreedyStrategy(this, s)),
                        new PhaseStrategy.Phase(0.0000, new CollectorStrategy(this, s)))));
                        //new PhaseStrategy.Phase(0.000, new VerticalSweepStrategy(this, s)))));
            }
        }
